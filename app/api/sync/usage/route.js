...
export async function POST(request) {
  try {
    ...
    const {
      userId, date,
      screenTimeMinutes = 0, unlockCount = 0,
      idleWorthScore = 10.0, aiInsight = null, appUsage = [],
    } = body;
    ...
    return NextResponse.json({ success: true, date, idleWorthScore }, { status: 200 });

  } catch (err) {
    console.error("[sync/usage POST]", err);
